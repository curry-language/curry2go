...
func ExternalDebug_Profile_getProcessInfos(task *gocurry.Task){
    ...
    elapsed_node := Prelude.Prelude__CREATE_LbCommaRb(root.NewNode(), Debug_DOT_Profile__CREATE_ElapsedTime(root.NewNode()), elapsed)
    ...
    sys_node := Prelude.Prelude__CREATE_LbCommaRb(root.NewNode(), Debug_DOT_Profile__CREATE_Memory(root.NewNode()), sys)
    
    // get stack memory
    stack := gocurry.IntLitCreate(root.NewNode(), int(mem_stats.StackSys))
    stack_node := Prelude.Prelude__CREATE_LbCommaRb(root.NewNode(), Debug_DOT_Profile__CREATE_Stack(root.NewNode()), stack)
    
    // get heap memory
    heap := gocurry.IntLitCreate(root.NewNode(), int(mem_stats.HeapAlloc))
    heap_node := Prelude.Prelude__CREATE_LbCommaRb(root.NewNode(), Debug_DOT_Profile__CREATE_Heap(root.NewNode()), heap)
    
    // get garbage collection count
    gc := gocurry.IntLitCreate(root.NewNode(), int(mem_stats.NumGC))
    gc_node := Prelude.Prelude__CREATE_LbCommaRb(root.NewNode(), Debug_DOT_Profile__CREATE_GarbageCollections(root.NewNode()), gc)
