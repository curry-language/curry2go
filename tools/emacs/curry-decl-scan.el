;;; curry-decl-scan.el --- Declaration scanning module for Curry Mode

;; Copyright (C) 1997-1998 Graeme E Moss

;; Authors: 1997-1998 Graeme E Moss <gem@cs.york.ac.uk>
;; Keywords: declarations menu files Curry
;; Version: 1.2
;; URL: http://www.cs.york.ac.uk/~gem/curry-mode/decl-scan.html
...
;; full Latin1 Curry 1.4 as well as literate scripts.
;;
;;
;; Installation:
;; 
;; To turn declaration scanning on for all Curry buffers under the
;; Curry mode of Moss&Thorn, add this to .emacs:
;;
;;    (add-hook curry-mode-hook 'turn-on-curry-decl-scan)
;;
;; Otherwise, call `turn-on-curry-decl-scan'.
;;
;;
;; Customisation:
;;
...
;;
;; . Re-running (literate-)curry-imenu should not cause the problems
;;   that it does.  The ability to turn off scanning would also be
;;   useful.  (Note that re-running (literate-)curry-mode seems to
;;   cause no problems.)
;;
...
;;; `(turn-(on/off)-)curry-decl-scan' or `curry-ds-'.

;;; The imenu support is based on code taken from `pakcs-mode',
;;; thanks go to Chris Van Humbeeck.

;; Version.
(defconst curry-decl-scan-version "1.1"
  "curry-decl-scan version number.")
(defun curry-decl-scan-version ()
  "Echo the current version of curry-decl-scan in the minibuffer."
  (interactive)
  (message "Using curry-decl-scan version %s" curry-decl-scan-version))
...
(defvar curry-ds-running-xemacs
...
(defvar curry-ds-syntax-table nil
  "Syntax table used for Curry declaration scanning.")

(if (not curry-ds-syntax-table)
    (progn
      (setq curry-ds-syntax-table (make-syntax-table))
  (modify-syntax-entry ?\  " " curry-ds-syntax-table)
  (modify-syntax-entry ?\t " " curry-ds-syntax-table)
  (modify-syntax-entry ?\" "\"" curry-ds-syntax-table)
  (modify-syntax-entry ?\' "w" curry-ds-syntax-table)
  (modify-syntax-entry ?_  "w" curry-ds-syntax-table)
  (modify-syntax-entry ?\( "()" curry-ds-syntax-table)
  (modify-syntax-entry ?\) ")(" curry-ds-syntax-table)
  (modify-syntax-entry ?[  "(]" curry-ds-syntax-table)
  (modify-syntax-entry ?]  ")[" curry-ds-syntax-table)
  (modify-syntax-entry ?{  "(}1" curry-ds-syntax-table)
  (modify-syntax-entry ?}  "){4" curry-ds-syntax-table)
  (modify-syntax-entry ?-  "_ 23" curry-ds-syntax-table)
  (modify-syntax-entry ?\` "$`" curry-ds-syntax-table)
  (mapcar (lambda (x)
            (modify-syntax-entry x "_" curry-ds-syntax-table))
          (concat "!#$%&*+./:<=>?@\\^|~"
                  (enum-from-to ?\241 ?\277)
                  "\327\367"))
  (mapcar (lambda (x)
            (modify-syntax-entry x "w" curry-ds-syntax-table))
          (concat (enum-from-to ?\300 ?\326)
                  (enum-from-to ?\330 ?\337)
                  (enum-from-to ?\340 ?\366)
                  (enum-from-to ?\370 ?\377)))))

(defun curry-ds-get-variable (prefix)
...
otherwise returns nil.  Point is not moved in either case."
  ...
  (let ((orig-table (syntax-table))
	...
	par-start
	name)
    (set-syntax-table curry-ds-syntax-table)
    (re-search-forward (concat "\\=" prefix) (point-max) t)
    ;; Keyword.
    (if (looking-at curry-ds-start-keywords-re)
	()
      (if (looking-at "(\\(\\s_+\\))")
	  ;; Paranthesised symbolic variable.
	  (setq name (curry-ds-match-string 1))
	(if (if (looking-at "(")
	    ...
	    (let ((match2 (curry-ds-match-string 2)))
		...
		(if (eq (aref match2 0) ?\`)
		    ...
		    (setq name (curry-ds-match-string 3))
		  ;; Infix symbolic variable.
		  (setq name match2)))))
	;; Variable identifier.
	(if (and (not name) (looking-at "\\sw+"))
	    (setq name (curry-ds-match-string 0)))))
    ;; Return the result.
    (goto-char start)
    (set-syntax-table orig-table)
    name))

(defun curry-ds-move-to-start-regexp (inc regexp)
...
(defvar literate-curry-ds-line-prefix "> "
...
(defvar curry-ds-start-decl-re "\\(\\sw\\|(\\)"
  "The regexp that starts a Curry declaration.")

(defvar literate-curry-ds-start-decl-re
  (concat literate-curry-ds-line-prefix curry-ds-start-decl-re)
  "The regexp that starts a Bird-style literate Curry declaration.")

(defun curry-ds-move-to-decl (direction bird-literate fix)
...
then point does not move if already at the start of a declaration."
  ;; As `curry-ds-get-variable' cannot separate an infix variable
  ...
  (let (;; The variable typed or bound in the current series of
	...
	abyss
	(line-prefix (if bird-literate literate-curry-ds-line-prefix ""))
	;; The regexp to match for the start of a declaration.
	(start-decl-re (if bird-literate
			   literate-curry-ds-start-decl-re
			 curry-ds-start-decl-re))
	...
	(if (and start (bobp))
	  ...
	  (curry-ds-move-to-start-regexp -1 start-decl-re))))
	...
	(if (not direction)
      ...
      (setq name (curry-ds-get-variable line-prefix))
      (if (not name)
	...
	(curry-ds-move-to-start-regexp increment start-decl-re)
	(while (and (/= (point) bound)
		    (and (setq newname (curry-ds-get-variable line-prefix))
			 (string= name newname)))
	  (setq name newname)
	  (curry-ds-move-to-start-regexp increment start-decl-re))
...
of the buffer and no declaration starts there."
  (interactive)
  (curry-ds-move-to-decl nil
                           (if (boundp 'curry-literate)
                               (eq curry-literate 'bird) nil)
                           nil))

(defun curry-ds-forward-decl ()
  "As `curry-ds-backward-decl' but forward."
  (interactive)
  (curry-ds-move-to-decl t
                           (if (boundp 'curry-literate)
                               (eq curry-literate 'bird) nil)
                           nil))

(defun curry-ds-generic-find-next-decl (bird-literate)
...
`class', `import' and `instance'."
  (let (;; The name, type and name-position of the declaration to
    ...
    (while (not (or (eobp) name))
      ...
      (curry-ds-move-to-decl t bird-literate t)
      ...
      (cond
       ...
       ;; grabbed.
       ((not (looking-at curry-ds-start-keywords-re))
	(setq name (curry-ds-get-variable ""))
	(if name
	    (progn
	      (setq type 'variable)
	...
	(setq name (curry-ds-match-string 2))
...
(defun curry-ds-create-imenu-index ()
  "Non-literate Curry version of `curry-ds-generic-create-menu-index'."
  (curry-ds-generic-create-imenu-index nil))

(defun literate-curry-ds-create-imenu-index ()
  "Bird-style literate Curry version of
`curry-ds-generic-create-menu-index'."
  (curry-ds-generic-create-imenu-index t))

(defun curry-ds-generic-create-imenu-index (bird-literate)
  "Function for finding imenu declarations in (BIRD-LITERATE) Curry mode.
Finds all declarations (classes, variables, imports, instances and
datatypes) in a Curry file for the imenu package."
  ...
  (let* ((index-alist '())
    ...
    (while (< (point) (point-max))
      ...
      (setq result (curry-ds-generic-find-next-decl bird-literate))
      (if result
    ...
    (and index-type-alist
	 (push (cons "Datatypes"
		     (sort index-type-alist 'curry-ds-imenu-label-cmp))
	       index-alist))
    (and index-inst-alist
	 (push (cons "Instances"
		     (sort index-inst-alist 'curry-ds-imenu-label-cmp))
	       index-alist))
    (and index-imp-alist
	 (push (cons "Imports"
		     (sort index-imp-alist 'curry-ds-imenu-label-cmp))
	       index-alist))
    (and index-var-alist
	 (push (cons "Variables"
		     (sort index-var-alist 'curry-ds-imenu-label-cmp))
	       index-alist))
    (and index-class-alist
	 (push (cons "Classes"
		     (sort index-class-alist 'curry-ds-imenu-label-cmp))
	       index-alist))
    (message "Sorting declarations in %s...done" bufname)
    ;; Return the alist.
    index-alist))

(defun curry-ds-imenu-label-cmp (el1 el2)
  "Predicate to compare labels in lists produced by
`curry-ds-create-imenu-index'."
  (string< (car el1) (car el2)))

(defun curry-ds-imenu (bird-literate)
  "Install imenu for (BIRD-LITERATE) Curry scripts."
  ...
  (if (and  (not curry-ds-running-xemacs)
	    (>= (string-to-number (substring emacs-version 0 2)) 20)
	    (not (fboundp 'imenu-extract-index-name-function)))
    (setq imenu-extract-index-name-function
	  (if bird-literate (function literate-curry-ds-create-imenu-index)
	    (function curry-ds-create-imenu-index))))
...
(defun curry-ds-func-menu-next (buffer)
  "Non-literate Curry version of `curry-ds-generic-func-menu-next'." 
  (curry-ds-generic-func-menu-next nil buffer)) 

(defun literate-curry-ds-func-menu-next (buffer)
  "Bird-style literate Curry version of `curry-ds-generic-func-menu-next'."
  (curry-ds-generic-func-menu-next t buffer)) 

(defun curry-ds-generic-func-menu-next (bird-literate buffer)
  "Returns `(name . pos)' of next declaration."
  (set-buffer buffer)
  (let ((result (curry-ds-generic-find-next-decl bird-literate)))
    (if result
	(let* ((name-posns (car result))
	       (name (car name-posns))
	       (posns (cdr name-posns))
...
(defvar curry-ds-func-menu-regexp
  (concat "^" curry-ds-start-decl-re)
  "Regexp to match the start of a possible declaration.")

(defvar literate-curry-ds-func-menu-regexp
  (concat "^" literate-curry-ds-start-decl-re)
  "As `curry-ds-func-menu-regexp' but for Bird-style literate scripts.")

(defun curry-ds-func-menu (bird-literate)
...
Curry scripts."
  ...
  (setq fume-function-name-regexp-alist
	(if bird-literate
            '((curry-mode . literate-curry-ds-func-menu-regexp))
          '((curry-mode . curry-ds-func-menu-regexp))))
  (make-local-variable 'fume-find-function-name-method-alist)
  (setq fume-find-function-name-method-alist
        (if bird-literate
            '((curry-mode . literate-curry-ds-func-menu-next))
          '((curry-mode . curry-ds-func-menu-next))))
...
(defun curry-ds-keys ()
  ...
  (local-set-key "\M-\C-e" 'curry-ds-forward-decl)
  (local-set-key "\M-\C-a" 'curry-ds-backward-decl))

;; The main functions to turn on declaration scanning.
(defun turn-on-curry-decl-scan ()
  "Turn on declaration scanning for Curry mode.  Top-level
...
\\[curry-ds-forward-decl] and \\[curry-ds-backward-decl] move forward and backward to the start of a declaration.
...
This may link with `curry-doc' (only for FSF Emacs currently).
...
declaration.  Therefore, using Curry font locking with comments
coloured in `font-lock-comment-face' improves declaration scanning.

To turn on declaration scanning for all Curry buffers, add this to
.emacs:
...
Literate Curry scripts are supported: If the value of
`curry-literate' (automatically set by the Curry mode of
...
Invokes `curry-decl-scan-hook' if not nil.

Use `curry-decl-scan-version' to find out what version this is."
  (interactive)
  (curry-ds-keys)
  (let ((bird-literate (if (boundp 'curry-literate)
                           (eq curry-literate 'bird) nil)))
    (if curry-ds-running-xemacs
        (curry-ds-func-menu bird-literate)
      (curry-ds-imenu bird-literate)))
  (run-hooks 'curry-decl-scan-hook))

;;; Provide ourselves:

(provide 'curry-decl-scan)

;;; curry-decl-scan ends here.
